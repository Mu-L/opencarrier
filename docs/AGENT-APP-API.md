...
- **WeChat OA 端点细则**：只代理服务器绑定账号（`senders/<app_id>/session.json` 存在且
  ...
  0=全部/1=普通/2=精选、`offset`、`count`≤50）。datacube 端点 `/datacube/article`（按发表日查单篇累计）与 `/datacube/summary`
  （账号级日汇总 getbizsummary：`{date}` 单日或 `{begin_date,end_date}` ≤30 天，T+1，
  daily-admin-brief 数据源）；
