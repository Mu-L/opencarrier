...
| # | 条目 | 优先级 | 改动量 | 收益 | 对应问题 | 状态 |
|---|------|--------|--------|------|----------|------|
| O1 | 引入 LoopState 统一状态管理 | 🔴 P0 | 中 | 消除状态散落，为后续优化打基础 | P9, P10 | ✅ 已完成 |
| O2 | 每轮注入 loop status（含上下文/时间预警） | 🔴 P0 | 小 | 模型决策信息完整，早期预警 | P2, P3 | ✅ 已完成 |
| O3 | 清理 modality 冗余代码 | 🟡 P1 | 极小 | 代码清晰度 | P1 | ✅ 已完成 |
| O4 | Loop 检测窗口 6→4 + 软检测 | 🟡 P1 | 小 | 减少死循环浪费 | P4 | ✅ 已完成 |
| O5 | 错误跟踪改为滑动窗口 | 🟡 P1 | 小 | 更准确的升级警告 | P8 | ✅ 已完成 |
| O6 | 单轨消息 (消除双轨) | 🟡 P1 | 中 | 消除不一致 bug 源 | P6 | ✅ 已完成 |
| O7 | Loop 状态机重构 | 🟢 P2 | 大 | 清晰的执行流，便于扩展 | 架构 | ✅ 已完成 |
| O8 | LoopState 跨 session 持久化 | 🟢 P2 | 小 | 解决冷启动 | P9 | ✅ 已完成 |
| O9 | Turn log 可观测性 (逐轮 token) | 🟢 P2 | 小 | 成本分析、性能调优 | P11 | ✅ 已完成 |
| O10 | 文本恢复失败后 fallback 文本优化 | 🔵 P3 | 极小 | UX 改善 | P5 | ✅ 已完成 |
| O11 | 工具发现改为追加而非驱逐 | 🔵 P3 | 小 | 减少重复 tool_search | P7 | ✅ 已完成 |
| O12 | MCP 健康检查 + 指数退避优化 | 🔵 P3 | 中 | 日志噪音消除，退避更快收敛 | P12 | ✅ 已完成 |
