...
description: 将排版好的文章发到微信公众号草稿箱(自动封面+建草稿,人工审核后发布)
version: 11
tools:
  - file_read
---
...
将排版好的文章发到**微信公众号草稿箱**。**建草稿动作(生成封面→上传→建草稿)由后台确定性 handler 执行,不走 agent 工具链**——你只负责确认文件和发一个标记。这是 2026-07 改造后的稳定方案,取代之前反复失败的 `image_generate`/`mcp_wechat_oa_*` 工具链。

**只建草稿,不自动发布**:AI 生成的内容必须人工审核后再公开发布,所以系统只把文章(含封面)放进草稿箱,由人在公众号后台审核后点「发布」。你(和用户)都不要期望它自动上线。
...
若 `正文.html` 不存��,先回到 article-formatter 排版,不要发标记。
...
路径用你 `file_read` 时用的同一个相对路径(如 `output/.../正文.html`)。

## 标记之后会发生什么(不用你管)

后台 handler 自动:
1. 读 `正文.html` 正文 + 同名 `.md` 首行作标题
2. **生成封面图**(失败则取素材库第一张图,再失败才报错——草稿必须有封面)
3. **建草稿**(不发布)
4. 把结果(`✅ 草稿已建,待审核` + media_id,或 `❌ 失败原因`)作为新消息推给用户
...
- 标记前用一句话告知用户「正在发到草稿箱…」,标记会被自动剥离、用户看不到
- 明确告诉用户:**草稿建好后需要人工去公众号后台审核发布**(���是自动上线)
- 标记里的 app_id 必须正确(发错公众号就发错了)
- `正文.md` 首行必须是标题——这决定草稿/文章的标题
- 不要重复发标记;不要在标记里塞多余内容
