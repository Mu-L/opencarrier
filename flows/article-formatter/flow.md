...
`正文.md` 以 `<!-- META -->` HTML 注释头开头（5行），包含标题/作者/摘要/类型/流水线ID。

**排版时必须：**
1. **识别 META 头**：文件开头的 `<!-- ... -->` 块，包含 `META_TITLE`、`META_AUTHOR` 等字段
2. **剥离 META 头**：生成的 HTML 中**不包含** META 注释内容，从 `# 标题` 对应的 HTML 开始输出
3. **从 META 头提取文章类型**：`META_TYPE` 字段决定排版模式（行业分析/热点评论/产品文章/深度教程）
4. **META 头缺失时**：回退到首行 `# 标题` 判断，正常排版，不报错
...
**识别 META 头**（文件开头 `<!-- META_TITLE: ... -->`），从中提取：
- `META_TYPE` → 决定排版模式
- `META_TITLE` → 文章标题（排版时跳过，公众号有自己的标题系统）

**剥离 META 头**，从 `# 标题` 开始处理正文。

如果 META 头缺失，回退到首行 `# 标题` 判断文章类型，正常排版。
...
优先从 `META_TYPE` 获取文章类型。META 头缺失时根据内容特征判断：
...
### 核心规范
- **从 `<!-- META -->` 头提取文章类型和标题，排版 HTML 时剥离 META 头**
