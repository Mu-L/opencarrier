...
enum HubCommands {
    ...
    /// Push a running clone's definition layer to DupHub as a new version.
    /// Closes the loop: runtime evolution (knowledge_add / private flow_update /
    /// MEMORY) -> Hub. No-op if content unchanged vs Hub latest.
    Push {
        /// Agent name (workspace under workspaces/).
        name: String,
    },
...
fn config_log_level() -> String {
    let config_path = types::config::home_dir().join("config.toml");
...
fn cmd_start(config: Option<PathBuf>) {
    ...
    rt.block_on(async {
        ...
        if let Err(e) =
            api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
...
fn boot_kernel_error(e: &kernel::error::KernelError) {
    let msg = e.to_string();
...
fn cmd_doctor(json: bool, repair: bool) {
        ...
        let api_listen = {
            ...
            if cfg_path.exists() {
                std::fs::read_to_string(&cfg_path)
                    .ok()
                    .and_then(|s| toml::from_str::<types::config::KernelConfig>(&s).ok())
                    .map(|c| c.api_listen)
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
...
fn cmd_providers() {
    ...
    loop {
        ...
        // Save
        match kernel::dotenv::save_env_key(&env_var, &key) {
            Ok(()) => {
                // Test
                print!("  Testing... ");
                io::stdout().flush().unwrap();
...
async fn cmd_hub(cmd: HubCommands) {
    let config = kernel::config::load_config(None::<&std::path::Path>);
    ...
    let api_key = match clone::hub::read_api_key(&config.hub.api_key_env) {
        Ok(k) => k,
        Err(e) => {
            eprintln!("{e}");
            std::process::exit(1);
    ...
    match cmd {
        HubCommands::Search { query } => {
            ...
            match clone::hub::search_templates(&hub_url, &api_key, &q).await {
        ...
        HubCommands::Push { name } => {
            let workspace_dir = config.effective_workspaces_dir().join(&name);
            if !workspace_dir.is_dir() {
                eprintln!("工作区不存在: {}", workspace_dir.display());
                std::process::exit(1);
            }
            let hub_id = match read_hub_template_id(&workspace_dir) {
                Some(id) => id,
                None => {
                    eprintln!("未找到 hub_template_id。先运行: opencarrier hub link {name}");
                    std::process::exit(1);
                }
            };
...
async fn cmd_plugin(cmd: PluginCommands) {
    ...
    match cmd {
        ...
        PluginCommands::Install { name, version } => {
            ...
            if clone::hub::is_plugin_installed(&plugins_dir, &name) {
                println!("插件 '{}' 已安装", name);
                return;
            }
        ...
        PluginCommands::Search { query } => {
            ...
            let q = query.unwrap_or_default();
            match clone::hub::search_plugins(&hub_url, &api_key, &q).await {
                Ok(result) => {
...
mod tests {
    ...
    fn test_doctor_config_deser_default() {
        ...
        let config = types::config::KernelConfig::default();
        let toml_str = toml::to_string_pretty(&config).unwrap();
        let parsed: types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
"#;
        let config: types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            types::config::ExecSecurityMode::Allowlist
        );
...
fn cmd_mcp_install(source: &str) {
    use kernel::mcp_registry;
    use types::mcp_manifest::McpServerManifest;
...
fn cmd_mcp_docker_start(name: &str) {
    use kernel::mcp_docker;
    let dir = kernel::mcp_registry::mcp_base_dir().join(name);
    if !dir.exists() {
