...
"#));
    ...
    let _ = kernel::dotenv::save_env_key("OPENCLONE_HUB_KEY", &api_key);
...
async fn register_and_get_key(
    ...
    let jwt = match clone::hub::register(hub_url, username, email, password).await {
        Ok(token) => token,
        Err(e) => {
    ...
    clone::hub::create_api_key(hub_url, &jwt, "carrier")
        .await
...
async fn login_and_get_key(
    ...
    let jwt = clone::hub::login(hub_url, username, password)
        .await
