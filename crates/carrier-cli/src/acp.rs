...
//!
//! Implements the standard ACP protocol over JSON-RPC 2.0, enabling carrier
...
use carrier_memory::acp_session::AcpSessionStore;
...
/// An active ACP session, mapping sessionId to an carrier agent.
#[allow(dead_code)]
...
pub fn handle_acp_request(
    ...
    match req.method.as_str() {
        ...
        "authenticate" => {
            // carrier doesn't require authentication
...
/// session/new — create a new session, map to an carrier agent.
fn acp_session_new(
    kernel: &Arc<CarrierKernel>,
    req: &JsonRpcRequest,
    state: &mut AcpConnectionState,
...
/// session/set_mode — accept mode switch (carrier doesn't distinguish internally).
...
/// _aginx/listAgents — list carrier agents.
fn acp_aginx_list_agents(kernel: &Arc<CarrierKernel>, req: &JsonRpcRequest) -> Option<Response> {
...
fn resolve_agent_id(kernel: &CarrierKernel, params: Option<&Value>) -> Result<AgentId, String> {
    let explicit = params
