//! Standalone chat TUI for `carrier chat`.
//!
...
enum Backend {
    ...
    InProcess { kernel: Arc<CarrierKernel> },
    None,
}
...
impl StandaloneChat {
    ...
    fn handle_stream_done(
        &mut self,
        result: Result<carrier_runtime::agent_loop::AgentLoopResult, String>,
    ) {
    ...
    fn handle_kernel_ready(&mut self, kernel: Arc<CarrierKernel>) {
        self.booting = false;
    ...
    fn resolve_daemon_agent(&mut self, base_url: &str, agent_name: Option<&str>) {
        ...
        match template {
            ...
            None => {
                self.boot_error = Some("No agent templates found. Run `carrier init`.".to_string());
            }
        }
    }
    ...
    fn resolve_inprocess_agent(&mut self) {
        ...
        match template {
            Some(t) => {
                let manifest: carrier_types::agent::AgentManifest = match toml::from_str(&t.content)
                {
                    Ok(m) => m,
                    Err(e) => {
            ...
            None => {
                self.chat.status_msg =
                    Some("No agent templates found. Run `carrier init`.".to_string());
            }
        }
    }
