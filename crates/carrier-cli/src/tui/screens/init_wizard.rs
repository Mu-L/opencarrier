...
//!
//! Launched by `carrier init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use carrier_runtime::model_catalog::ModelCatalog;
...
impl State {
    ...
    fn build_provider_order(&mut self) {
        ...
        for (i, p) in PROVIDERS.iter().enumerate() {
            let detected = if p.name == "claude-code" {
                carrier_runtime::drivers::claude_code::claude_code_available()
            } else {
    ...
    fn is_provider_detected(&self, prov_idx: usize) -> bool {
        ...
        if p.name == "claude-code" {
            return carrier_runtime::drivers::claude_code::claude_code_available();
        }
        (!p.env_var.is_empty() && std::env::var(p.env_var).is_ok())
                            ...
                            KeyCode::Enter => {
                                    ...
                                    && state.key_test == KeyTestState::Idle
                                {
                                    if let Some(p) = state.provider() {
                                        let _ = carrier_kernel::dotenv::save_env_key(
                                            p.env_var,
                                            &state.api_key_input,
                                        );
                                    }
...
fn save_config(state: &mut State) {
    ...
    let carrier_dir = if let Ok(h) = std::env::var("CARRIER_HOME") {
        PathBuf::from(h)
    } else {
        match dirs::home_dir() {
            Some(h) => h.join(".carrier"),
            None => {
    ...
    let _ = std::fs::create_dir_all(carrier_dir.join("agents"));
    let _ = std::fs::create_dir_all(carrier_dir.join("data"));
    crate::restrict_dir_permissions(&carrier_dir);
...
/// Check if the `carrier-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "Carrier Init  Step X of 6"
...
fn draw_routing_pick(f: &mut Frame, area: Rect, state: &mut State, tier: usize) {
        ...
        .enumerate()
    {
        if t == tier {
            ...
            let display = carrier_types::truncate_str(short, 14);
            summary_spans.push(Span::styled(
                format!("{name}:{display}"),
                Style::default().fg(*c),
            ));
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use Carrier?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
