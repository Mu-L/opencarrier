...
fn cmd_start(config: Option<PathBuf>) {
    ...
    rt.block_on(async {
        let kernel_config = carrier_kernel::config::load_config(config.as_deref());
        ...
        if let Err(e) =
            carrier_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
...
fn read_api_key() -> Option<String> {
    ...
    let config_path = cli_carrier_home().join("config.toml");
...
fn boot_kernel_error(e: &carrier_kernel::error::KernelError) {
    let msg = e.to_string();
...
fn cmd_chat_once(
    ...
    let kernel = match CarrierKernel::boot(config_path.as_deref()) {
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(_h) = &home {
        ...
        if config_path.exists() {
            ...
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                ...
                let _ = std::fs::create_dir_all(&carrier_dir);
        ...
        let api_listen = {
            let cfg_path = carrier_dir.join("config.toml");
            if cfg_path.exists() {
                std::fs::read_to_string(&cfg_path)
                    .ok()
                    .and_then(|s| toml::from_str::<carrier_types::config::KernelConfig>(&s).ok())
                    .map(|c| c.api_listen)
        ...
        let daemon_json_path = carrier_dir.join("daemon.json");
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &carrier_dir.display().to_string()])
                .output()
            {
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<carrier_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = carrier_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                carrier_types::config::McpTransportEntry::Stdio {
                                    command, ..
                                } => {
                                    if command.is_empty() {
                                        if !json {
                                ...
                                carrier_types::config::McpTransportEntry::Sse { url } => {
                                    if url.is_empty() {
                                        if !json {
                                            ui::check_warn(&format!(
...
fn boot_kernel(config: Option<PathBuf>) -> CarrierKernel {
    match CarrierKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_providers() {
    ...
    let brain: carrier_types::brain::BrainConfig = serde_json::from_str(&brain_content)
        .unwrap_or_else(|e| {
    ...
    loop {
        ...
        // Save
        match carrier_kernel::dotenv::save_env_key(&env_var, &key) {
            Ok(()) => {
                // Test
                print!("  Testing... ");
                io::stdout().flush().unwrap();
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = carrier_runtime::model_catalog::ModelCatalog::new();
...
fn pick_model() -> String {
    ...
    let mut by_provider: std::collections::BTreeMap<
        String,
        Vec<&carrier_types::model_catalog::ModelCatalogEntry>,
...
fn cmd_reset(confirm: bool) {
    ...
    if !confirm {
        println!("  This will delete all data in {}", carrier_dir.display());
    ...
    match std::fs::remove_dir_all(&carrier_dir) {
        Ok(()) => ui::success(&format!("Removed {}", carrier_dir.display())),
        Err(e) => {
            ui::error(&format!("Failed to remove {}: {e}", carrier_dir.display()));
            std::process::exit(1);
        }
    }
}
...
async fn cmd_hub(cmd: HubCommands) {
    let config = carrier_kernel::config::load_config(None::<&std::path::Path>);
    ...
    let api_key = match carrier_clone::hub::read_api_key(&config.hub.api_key_env) {
        Ok(k) => k,
        Err(e) => {
            eprintln!("{e}");
            std::process::exit(1);
...
async fn cmd_plugin(cmd: PluginCommands) {
    ...
    match cmd {
        ...
        PluginCommands::Install { name, version } => {
            ...
            if carrier_clone::hub::is_plugin_installed(&plugins_dir, &name) {
                println!("插件 '{}' 已安装", name);
                return;
            }
        ...
        PluginCommands::Search { query } => {
            ...
            let q = query.unwrap_or_default();
            match carrier_clone::hub::search_plugins(&hub_url, &api_key, &q).await {
                Ok(result) => {
...
fn cmd_uninstall(confirm: bool, keep_config: bool) {
    ...
    if carrier_dir.exists() {
        if keep_config {
            ...
            println!("  • Remove {}", carrier_dir.display());
        }
    }
    ...
    if find_daemon().is_some() {
        ...
        if find_daemon().is_some() {
            if let Some(info) = read_daemon_info(&carrier_dir) {
                force_kill_pid(info.pid);
                let _ = std::fs::remove_file(carrier_dir.join("daemon.json"));
            }
        }
    }
    ...
    if carrier_dir.exists() {
        if keep_config {
            ...
            match std::fs::remove_dir_all(&carrier_dir) {
                Ok(()) => ui::success(&format!("Removed {}", carrier_dir.display())),
                Err(e) => ui::error(&format!("Failed to remove {}: {e}", carrier_dir.display())),
            }
        }
    }
...
fn remove_autostart_entries(home: &std::path::Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let service_file = home.join(".config/systemd/user/carrier.service");
        if service_file.exists() {
            let _ = std::process::Command::new("systemctl")
                .args(["--user", "disable", "--now", "carrier.service"])
                .output();
...
/// Remove lines from shell config files that add carrier to PATH.
#[allow(unused_variables)]
fn clean_path_entries(home: &std::path::Path, carrier_dir: &str) {
    #[cfg(not(windows))]
    {
        let shell_files = [
            home.join(".bashrc"),
        ...
        for path in &shell_files {
            ...
            let filtered: Vec<&str> = content
                .lines()
                .filter(|line| !is_carrier_path_line(line, carrier_dir))
                .collect();
    ...
    #[cfg(windows)]
    {
        // Read User PATH via PowerShell, filter out carrier entries, write back
        ...
        if let Ok(out) = output {
            if out.status.success() {
                ...
                if !current.is_empty() {
                    let dir_lower = carrier_dir.to_lowercase();
                    let filtered: Vec<&str> = current
                        .split(';')
                        .filter(|entry| {
                            let e = entry.trim().to_lowercase();
                            !e.is_empty() && !e.contains("carrier") && !e.contains(&dir_lower)
                        })
                        .collect();
...
/// Returns true if a shell config line is an carrier PATH export.
/// Must match BOTH an carrier reference AND a PATH-setting pattern.
#[cfg(any(not(windows), test))]
fn is_carrier_path_line(line: &str, carrier_dir: &str) -> bool {
    let lower = line.to_lowercase();
    let has_carrier = lower.contains("carrier") || lower.contains(&carrier_dir.to_lowercase());
    if !has_carrier {
        return false;
    }
...
mod tests {
    ...
    fn test_doctor_config_deser_default() {
        ...
        let config = carrier_types::config::KernelConfig::default();
        let toml_str = toml::to_string_pretty(&config).unwrap();
        let parsed: carrier_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
"#;
        let config: carrier_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            carrier_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
        ...
        match &config.mcp_servers[0].transport {
            carrier_types::config::McpTransportEntry::Stdio { command, args } => {
    ...
    fn test_uninstall_path_line_filter() {
        ...
        assert!(is_carrier_path_line(
            r#"export PATH="/home/user/.carrier/bin:$PATH""#,
            dir
        ));
        assert!(is_carrier_path_line(
        ...
        // Should NOT match: carrier lines that aren't PATH-related
        assert!(!is_carrier_path_line("# carrier config", dir));
        assert!(!is_carrier_path_line("alias of=carrier", dir));
    }
}
...
fn cmd_mcp_install(source: &str) {
    use carrier_kernel::mcp_registry;
    use carrier_types::mcp_manifest::McpServerManifest;
...
fn cmd_mcp_docker_start(name: &str) {
    use carrier_kernel::mcp_docker;
    let dir = carrier_kernel::mcp_registry::mcp_base_dir().join(name);
    if !dir.exists() {
