        ...
        Ok(Some(session)) => {
            ...
            for m in &session.messages {
                ...
                let content = match &m.content {
                    types::message::MessageContent::Text(t) => t.clone(),
                    types::message::MessageContent::Blocks(blocks) => {
                        let mut texts = Vec::new();
                        for b in blocks {
                            match b {
                                types::message::ContentBlock::Text { text, .. } => {
                                ...
                                types::message::ContentBlock::ToolResult { .. } => {}
                                _ => {}
                            }
                        }
                        texts.join("\n")
            ...
            for m in &session.messages {
                if let types::message::MessageContent::Blocks(blocks) = &m.content {
                    for b in blocks {
                        if let types::message::ContentBlock::ToolResult {
                            tool_use_id,
                            content: result,
                            is_error,
                            ..
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
