...
pub async fn comms_topology(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    use types::comms::{EdgeKind, TopoEdge, TopoNode, Topology};
    ...
    for event in &events {
        if let types::event::EventPayload::Message(_) = &event.payload {
            if let types::event::EventTarget::Agent(target_id) = &event.target {
...
fn filter_to_comms_event(
    ...
    agents: &[types::agent::AgentEntry],
) -> Option<types::comms::CommsEvent> {
    use types::comms::{CommsEvent, CommsEventKind};
    use types::event::{EventPayload, EventTarget, LifecycleEvent};
    ...
    match &event.payload {
        EventPayload::Message(msg) => {
            ...
            Some(CommsEvent {
                ...
                detail: types::truncate_str(&msg.content, 200).to_string(),
            })
        }
...
fn audit_to_comms_event(
    entry: &runtime::audit::AuditEntry,
    agents: &[types::agent::AgentEntry],
) -> Option<types::comms::CommsEvent> {
    use types::comms::{CommsEvent, CommsEventKind};
        ...
        agents
            ...
            .unwrap_or_else(|| {
                if id.is_empty() || id == "system" {
                    ...
                    types::truncate_str(id, 12).to_string()
                }
            })
    };
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!(
                        ...
                        in_tok,
                        out_tok,
                        types::truncate_str(&entry.outcome, 80)
                    )
                }
            } else if entry.outcome != "ok" {
                format!(
                    "{} -- {}",
                    types::truncate_str(&entry.detail, 80),
                    types::truncate_str(&entry.outcome, 80)
                )
            } else {
                types::truncate_str(&entry.detail, 200).to_string()
            };
...
pub async fn comms_events(
    ...
    let mut comms_events: Vec<types::comms::CommsEvent> = bus_events
        .iter()
...
pub async fn comms_task(
    State(state): State<Arc<AppState>>,
    Json(req): Json<types::comms::CommsTaskRequest>,
) -> impl IntoResponse {
