...
fn test_chat_request_deserialize() {
    ...
    let request: carrier_types::conversation::ChatRequest = serde_json::from_value(json).unwrap();

    assert_eq!(request.msg_type, "chat");
    assert_eq!(request.conversation_id, "conv-002");
    assert_eq!(
        request.conversation_type,
        carrier_types::conversation::ConversationType::Carrier
    );
    assert_eq!(
        request.chat_type,
        carrier_types::conversation::ChatType::Direct
    );
...
fn test_chat_request_plugin_mode() {
    ...
    assert_eq!(
        request.conversation_type,
        carrier_types::conversation::ConversationType::Plugin
    );
...
fn test_chat_response_serialize() {
    use carrier_types::conversation::{ChatRequest, ChatType, ConversationType};
    ...
    let response = carrier_types::conversation::ChatResponse::for_request(
        &request,
...
fn test_chat_response_with_metadata() {
    use carrier_types::conversation::{ChatRequest, ChatType, ConversationType, ResponseMetadata};
    ...
    let response =
        carrier_types::conversation::ChatResponse::for_request(&request, "Done".to_string())
...
fn test_error_response_serialize() {
    let error = carrier_types::conversation::ErrorResponse::new("Something went wrong");
...
fn test_error_response_for_request() {
    ...
    let error =
        carrier_types::conversation::ErrorResponse::for_request(&request, "Test error".to_string());
