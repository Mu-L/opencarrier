...
use axum::Router;
use carrier_api::middleware;
use carrier_api::routes::{self, AppState};
use carrier_api::server::{read_daemon_info, DaemonInfo};
...
async fn test_server_immediate_responsiveness() {
    ...
    let kernel = CarrierKernel::boot_with_config(config).unwrap();
    let kernel = Arc::new(kernel);
