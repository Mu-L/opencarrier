        ...
        Ok(Some(session)) => {
            ...
            for m in &session.messages {
                ...
                let content = match &m.content {
                    ...
                    carrier_types::message::MessageContent::Blocks(blocks) => {
                        ...
                        for b in blocks {
                            match b {
                                ...
                                carrier_types::message::ContentBlock::ToolUse {
                                    id,
                                    name,
                                    input,
                                    ..
                                ...
                                carrier_types::message::ContentBlock::ToolResult { .. } => {}
                                _ => {}
                            }
                        }
                        texts.join("\n")
            ...
            for m in &session.messages {
                if let carrier_types::message::MessageContent::Blocks(blocks) = &m.content {
                    for b in blocks {
                        if let carrier_types::message::ContentBlock::ToolResult {
                            tool_use_id,
                            content: result,
                            is_error,
                            ..
...
pub async fn delete_session(
    ...
    let session_id = match id.parse::<uuid::Uuid>() {
        Ok(u) => carrier_types::agent::SessionId(u),
        Err(_) => {
            return (
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = carrier_types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => carrier_types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
