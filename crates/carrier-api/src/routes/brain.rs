...
pub async fn set_brain_provider(
    ...
    if let Some(api_key) = body["api_key"].as_str() {
        if !api_key.is_empty() && !api_key_env.is_empty() {
            let _ = carrier_kernel::dotenv::save_env_key(&api_key_env, api_key);
        }
    }
    ...
    let result = state.kernel.update_brain(|config| {
        config.providers.insert(
            name.clone(),
            carrier_types::brain::ProviderConfig {
                api_key_env,
                auth_type,
                params,
            },
...
pub async fn set_brain_endpoint(
    ...
    let format: carrier_types::brain::ApiFormat = match serde_json::from_value(
        ...
        Err(_) => {
            return (
                ...
                Json(serde_json::json!({"error": format!(
                    "Invalid format '{}'. Supported: openai, anthropic, gemini, dashscope_tts, dashscope_image, dashscope_video, kling, openai_images",
                    format_str
                )})),
            )
        }
    ...
    let result = state.kernel.update_brain(|config| {
        config.endpoints.insert(
            name.clone(),
            carrier_types::brain::EndpointConfig {
                provider,
                model,
                base_url,
                format,
...
pub async fn set_brain_modality(
    ...
    let result = state.kernel.update_brain(|config| {
        config.modalities.insert(
            name.clone(),
            carrier_types::brain::ModalityConfig {
                primary,
                fallbacks,
                description,
            },
...
pub async fn put_brain_config_raw(
    ...
    let config: carrier_types::brain::BrainConfig = match serde_json::from_value(body.clone()) {
        Ok(c) => c,
        Err(e) => {
            return (
                StatusCode::BAD_REQUEST,
