...
pub async fn comms_topology(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    use carrier_types::comms::{EdgeKind, TopoEdge, TopoNode, Topology};
    ...
    for event in &events {
        if let carrier_types::event::EventPayload::Message(_) = &event.payload {
            if let carrier_types::event::EventTarget::Agent(target_id) = &event.target {
...
fn filter_to_comms_event(
    event: &carrier_types::event::Event,
    agents: &[carrier_types::agent::AgentEntry],
) -> Option<carrier_types::comms::CommsEvent> {
    use carrier_types::comms::{CommsEvent, CommsEventKind};
    use carrier_types::event::{EventPayload, EventTarget, LifecycleEvent};
    ...
    match &event.payload {
        EventPayload::Message(msg) => {
            ...
            Some(CommsEvent {
                ...
                detail: carrier_types::truncate_str(&msg.content, 200).to_string(),
            })
        }
...
fn audit_to_comms_event(
    entry: &carrier_runtime::audit::AuditEntry,
    agents: &[carrier_types::agent::AgentEntry],
) -> Option<carrier_types::comms::CommsEvent> {
    use carrier_types::comms::{CommsEvent, CommsEventKind};
        ...
        agents
            ...
            .unwrap_or_else(|| {
                if id.is_empty() || id == "system" {
                    ...
                    carrier_types::truncate_str(id, 12).to_string()
                }
            })
    };
    ...
    let (kind, detail, target_label) = match action_str.as_str() {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!(
                        ...
                        in_tok,
                        out_tok,
                        carrier_types::truncate_str(&entry.outcome, 80)
                    )
                }
            } else if entry.outcome != "ok" {
                format!(
                    "{} -- {}",
                    carrier_types::truncate_str(&entry.detail, 80),
                    carrier_types::truncate_str(&entry.outcome, 80)
                )
            } else {
                carrier_types::truncate_str(&entry.detail, 200).to_string()
            };
        ...
        "AgentSpawn" => (
            ...
            format!(
                "Agent spawned: {}",
                carrier_types::truncate_str(&entry.detail, 100)
            ),
            "",
        ),
        "AgentKill" => (
...
pub async fn comms_events(
    ...
    let mut comms_events: Vec<carrier_types::comms::CommsEvent> = bus_events
        .iter()
...
pub async fn comms_send(
    State(state): State<Arc<AppState>>,
    Json(req): Json<carrier_types::comms::CommsSendRequest>,
) -> impl IntoResponse {
...
pub async fn comms_task(
    State(state): State<Arc<AppState>>,
    Json(req): Json<carrier_types::comms::CommsTaskRequest>,
) -> impl IntoResponse {
