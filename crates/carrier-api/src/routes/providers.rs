    ...
    let providers: Vec<serde_json::Value> = config
        ...
        .map(|(name, pc)| {
            ...
            let has_key = if pc.auth_type == "jwt" {
                ...
                pc.params
                    .values()
                    .all(|env_name| carrier_kernel::dotenv::has_env_key(env_name))
            } else {
                carrier_kernel::dotenv::has_env_key(&pc.api_key_env)
            };
            ...
            let params_status: Vec<serde_json::Value> = pc
                ...
                .map(|(logical_name, env_name)| {
                    serde_json::json!({
                        "name": logical_name,
                        "env": env_name,
                        "has_value": carrier_kernel::dotenv::has_env_key(env_name),
                    })
                })
                .collect();
...
pub async fn set_provider_key(
    ...
    match pc.auth_type.as_str() {
        "jwt" => {
            ...
            for (logical_name, env_name) in &pc.params {
                if let Some(val) = params.get(logical_name).and_then(|v| v.as_str()) {
                    ...
                    if let Err(e) = carrier_kernel::dotenv::save_env_key(env_name, val) {
                        return (
        ...
        _ => {
            ...
            if let Err(e) = carrier_kernel::dotenv::save_env_key(&pc.api_key_env, key) {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(serde_json::json!({"error": e})),
                );
...
pub async fn delete_provider_key(
    ...
    if pc.auth_type == "jwt" {
        ...
        for env_name in pc.params.values() {
            let _ = carrier_kernel::dotenv::delete_env_key(env_name);
        }
    } else {
        let _ = carrier_kernel::dotenv::delete_env_key(&pc.api_key_env);
    }
