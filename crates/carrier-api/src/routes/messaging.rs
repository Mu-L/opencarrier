...
) -> Vec<carrier_types::message::ContentBlock> {
    use base64::Engine;
    ...
    for att in attachments {
        ...
        match std::fs::read(&file_path) {
            Ok(data) => {
                ...
                blocks.push(carrier_types::message::ContentBlock::Image {
...
pub fn inject_attachments_into_session(
    ...
    agent_id: AgentId,
    image_blocks: Vec<carrier_types::message::ContentBlock>,
) {
    ...
    let mut session = match kernel.memory.get_session(entry.session_id) {
        Ok(Some(s)) => s,
        _ => carrier_memory::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
