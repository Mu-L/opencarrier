...
pub async fn mcp_http(
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn carrier_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn carrier_runtime::kernel_handle::KernelHandle>;
    ...
    let response = carrier_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
