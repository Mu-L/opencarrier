...
use carrier_types::memory::{Entity, EntityType, GraphMatch, GraphPattern, Relation, RelationType};
use chrono::Utc;
...
impl KnowledgeStore {
    ...
    pub fn add_entity(&self, entity: Entity) -> CarrierResult<String> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn add_relation(&self, relation: Relation) -> CarrierResult<String> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CarrierError::Internal(e.to_string()))?;
    ...
    pub fn query_graph(&self, pattern: GraphPattern) -> CarrierResult<Vec<GraphMatch>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CarrierError::Internal(e.to_string()))?;
        ...
        for row_result in rows {
            let r = row_result.map_err(|e| CarrierError::Memory(e.to_string()))?;
