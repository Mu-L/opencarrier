...
use carrier_runtime::llm_driver::{LlmDriver, StreamEvent};
use carrier_runtime::python_runtime::{self, PythonConfig};
use carrier_runtime::sandbox::{SandboxConfig, WasmSandbox};
use carrier_runtime::tool_runner::builtin_tool_definitions;
...
pub struct KernelBrain {
    ...
    pub model_catalog: std::sync::RwLock<carrier_runtime::model_catalog::ModelCatalog>,
}
...
pub struct KernelServices {
    ...
    pub web_ctx: carrier_runtime::web_search::WebToolsContext,
    /// Browser automation manager (Playwright bridge sessions).
    pub browser_ctx: carrier_runtime::browser::BrowserManager,
...
pub struct KernelPlugins {
    ...
    pub mcp_connections: dashmap::DashMap<String, carrier_runtime::mcp::McpConnection>,
...
pub struct KernelRuntime {
    ...
    watcher_handles: std::sync::Mutex<Vec<carrier_lifecycle::watcher::WatcherHandle>>,
}
...
pub struct KernelCoordination {
    ...
    self_handle: OnceLock<Weak<CarrierKernel>>,
}
...
impl CarrierKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        let mut model_catalog = carrier_runtime::model_catalog::ModelCatalog::new();
        ...
        let web_ctx = carrier_runtime::web_search::WebToolsContext {
            search: carrier_runtime::web_search::WebSearchEngine::new(web_cache.clone()),
            fetch: carrier_runtime::web_fetch::WebFetchEngine::new(
                config.web.fetch.clone(),
                web_cache,
            ),
            brain: Some(brain_arc.clone() as Arc<dyn carrier_runtime::llm_driver::Brain>),
        };

        let browser_ctx = carrier_runtime::browser::BrowserManager::new(config.browser.clone());
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        use carrier_runtime::compactor::{compact_session, needs_compaction, CompactionConfig};
    ...
    pub fn list_bindings(&self) -> Vec<carrier_types::config::AgentBinding> {
        self.coordination
            .bindings
            .lock()
    ...
    pub fn add_binding(&self, binding: carrier_types::config::AgentBinding) {
    ...
    pub fn remove_binding(&self, index: usize) -> Option<carrier_types::config::AgentBinding> {
        let mut bindings = self
            .coordination
            .bindings
            .lock()
    ...
    pub fn reload_brain(&self) -> Result<(), String> {
        ...
        let config: carrier_types::brain::BrainConfig =
    ...
    async fn connect_mcp_servers(self: &Arc<Self>) {
        use carrier_runtime::mcp::{McpConnection, McpServerConfig, McpTransport};
            ...
            loop {
                ...
                let mut dead_servers = Vec::new();
                {
                    ...
                    let dead_keys: Vec<String> = dead_servers
                        .iter()
                        .map(|(name, _)| carrier_runtime::mcp::normalize_name(name))
                        .collect();
...
mod tests {
    ...
    fn test_manifest_to_capabilities_with_profile() {
        use carrier_types::agent::ToolProfile;
        let manifest = AgentManifest {
