...
//! `types::env` so that env mutations from async code are safe.
//! All code that reads environment variables should use `types::env::get_env`
...
pub fn env_file_path() -> Option<PathBuf> {
    Some(types::config::home_dir().join(".env"))
}
...
pub fn secrets_env_path() -> Option<PathBuf> {
    Some(types::config::home_dir().join("secrets.env"))
}
...
fn load_env_file(path: Option<PathBuf>) {
    ...
    for line in content.lines() {
        ...
        if let Some((key, value)) = parse_env_line(trimmed) {
            if types::env::get_env(&key).is_none() {
                types::env::set_env_override(&key, &value);
            }
        }
    }
}
...
pub fn save_env_key(key: &str, value: &str) -> Result<(), String> {
    ...
    // Set in the in-process override map so types::env::get_env()
...
pub fn delete_env_key(key: &str) -> Result<(), String> {
    ...
    types::env::remove_env_override(key);
