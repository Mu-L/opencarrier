...
fn regenerate_config_snippet() -> Result<(), String> {
    ...
    for record in &enabled {
        ...
        match &manifest.runtime.transport {
            types::mcp_manifest::McpRuntimeTransport::Sse { url } => {
                toml.push_str(&format!(
                    "[[mcp_servers]]\n\
                     name = \"{name}\"\n\
            ...
            types::mcp_manifest::McpRuntimeTransport::Stdio { command, args } => {
