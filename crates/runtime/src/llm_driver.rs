...
pub struct CompletionRequest {
    ...
    pub thinking: Option<types::config::ThinkingConfig>,
...
pub trait Brain: Send + Sync {
    ...
    fn list_modalities(&self) -> Vec<types::brain::ModalityInfo> {
        vec![]
    }
    ...
    fn endpoints_for(&self, _modality: &str) -> Vec<types::brain::ResolvedEndpoint> {
        vec![]
    }
    ...
    fn report(&self, _report: types::brain::EndpointReport) {}

    /// Get current Brain status snapshot.
    fn status(&self) -> types::brain::BrainStatus {
        types::brain::BrainStatus {
            modalities: vec![],
            endpoints: vec![],
            drivers_ready: 0,
        }
