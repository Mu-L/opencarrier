...
| `shell_exec` | Shell 命令 |
| `web_fetch` | 网页抓取（命中风控站自动走 AginxBrower 渲染，见下） |
| `web_search` | 网页搜索（走 AginxBrower 聚合搜索，支持搜+读一步完成） |
...
OpenCarrier 把 [AginxBrower](https://github.com/yinnho/aginxbrower)（轻量服务端浏览器，内置 Obscura + V8，stealth 指纹）当作**外挂**接入：`web_fetch` 内部识别到风控站时，自动路由到 AginxBrower 渲染抓取，失败再回退 reqwest。**对 Agent 完全透明**——Agent 照常调 `web_fetch(url)`，参数和返回格式都不变，无需知道 AginxBrower 的存在。
...
需浏览器交互（点击、滚动、执行 JS）的场景，另有独立的 `browser_*` 工具集（`browser_navigate`/`browser_evaluate`/`browser_click` 等）显式调用 AginxBrower。

#### 网页搜索

`web_search` 内置搜索工具，调 AginxBrower 的 `/search` 端点做聚合搜索（后端 SearXNG），支持 `fetch_top` 参数对前 N 条结果自动抓正文——Agent 一步完成"搜→读"。共用 `AGINXBROWER_URL` 环境变量开关。
